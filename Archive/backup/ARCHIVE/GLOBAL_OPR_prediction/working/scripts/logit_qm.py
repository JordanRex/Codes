...
def quick_model_log(train, valid, ytrain, yvalid):
    ...
    logmodel.fit(train_new, ytrain)
    print(logmodel.score(valid_new, yvalid))
    
    pred_probs = logmodel.predict_proba(valid_new)
    pred = logmodel.predict(valid_new)
    
    return logmodel, pred, pred_probs