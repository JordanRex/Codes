...
def create_model(X, Y, XV, YV):
    ...
    model.add(Dense({{choice([50, 100, 250, 500, 1000, 2000])}}, activation={{choice(['relu', 'tanh'])}}))
    model.add(Dropout({{uniform(0, 0.7)}}))
    model.add(Dense({{choice([50, 100, 250, 500, 1000])}}, activation = {{choice(['relu', 'tanh'])}}))
    if {{choice(['true', 'false'])}} == 'true':
        model.add(Dense({{choice([5, 20, 30, 50, 100])}}, activation = {{choice(['relu', 'tanh'])}}))
    model.add(Dropout({{uniform(0, 0.7)}}))
    model.add(Dense(9, activation={{choice(['softmax'])}}))

    model.compile(loss='categorical_crossentropy', optimizer = {{choice(['adam', 'adadelta'])}}, 
                  metrics=['accuracy'])
