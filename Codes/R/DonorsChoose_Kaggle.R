...
{
  train = read.csv('/kaggle/input/train.csv', header = T, stringsAsFactors = F)
  test = read.csv('/kaggle/input/test.csv', header = T, stringsAsFactors = F)

  resources = read.csv('/kaggle/input/resources.csv', header = T, stringsAsFactors = F)
...
# The NLP segment
{
  ...
  # define the basic functions to be used across the various NLP feats
  {
    ...
    text2vec_fn = function(x1, x2, y, n) {
      ...
      vocab = create_vocabulary(it_train, stopwords = all_stopwords, ngram = c(1,4)) %>%
        prune_vocabulary(., term_count_min = 10, doc_proportion_max = 0.8, doc_proportion_min = 0.001, vocab_term_max = n)
  ...
  # train_test_nlp features - the NLP modelling module
  {
    ...
    # nlp feats to be taken stored as nlp_colnames_features (200 features alone)
    {
      train_test_nlp %<>%
...
# h2o deep learning
{
  ...
  # Train & Cross-validate a DNN
  {
    ...
    h2o_DstTrain[, length(names(h2o_DstTrain)) + 1] = ifelse(h2o_DstTrain[, 1] == 0, 4, 1)
